...
class AddIndexesForTasks extends Migration
{
    ...
    public function up()
    {
        Schema::table('admin_scheduling_task_results', function (Blueprint $table) {
                $table->index('task_id', 'task_results_task_id_idx');
                $table->index('ran_at', 'task_results_ran_at_idx');
                $table->foreign('task_id', 'task_id_fk')
                    ->references('id')
                    ->on('admin_scheduling_tasks');
            });

        Schema::table('admin_scheduling_task_frequencies', function (Blueprint $table) {
